...
| 审计线 | 执行者 | 检查内容 |
|--------|--------|---------|
| Word 格式检查 | Python + python-docx（38项） | 字体字号、标题层级/对齐/间距、页眉页脚、图表编号+中英文标题、公式编号+引用括号、参考文献、空行、标点混用、括号匹配、重复字词、公式源码残留、重复标点、引号配对 |
| PDF 空间检查 | Python + PyMuPDF（9项） | 页面尺寸、版芯（按章抽样）、图表题注位置（支持矢量图）、页底空白、图片分辨率、公式居中、式号右对齐、表格跨页 |
| 交叉引用检查 | Python + PyMuPDF | 引用存在性 + 未引用检测 + 先文后图/表顺序（含同页y坐标） |
...
| 公式 | 编号按章连续、引用风格统一（式vs公式）、引用括号必须半角 |
| 参考文献 | [N]顺序编号、GB/T 7714格式标识（[M][J][D]） |
| 段落 | 末行≥5字、连续空行检测、编号分隔符半角点 |
| 目录 | 标题一致性、编号连续性 |
| 低级错误 | 中英文标点混用、全角半角数字混用、标题末尾多余标点、连续重复字词（"的的"）、括号不匹配、大表格跨页属性、公式源码残留（$\\frac等LaTeX泄漏）、连续重复标点（，，等）、引号/书名号配对 |
...
| # | 检查项 |
|---|--------|
| 1 | 节引用存在性（"第X.Y节"指向真实标题） |
| 2 | 图引用存在性（"图X.Y"指向真实图题） |
| 3 | 表引用存在性（"表X.Y"指向真实表题） |
| 4 | 公式引用存在性（"式(X.Y)"指向真实公式） |
| 5 | 未引用检测（定义了但正文从未引用的图/表/公式） |
| 6 | 先文后图/表顺序（首次引用必须在定义之前，含同页y坐标比较） |
| 7 | 孤立公式检测（有编号但未被"如式(X.Y)所示"引用） |
...
输出按章节拆分的 Markdown 审计报告到 `output/` 目录（00-前置部分 ~ 10-边界文本检查）。
...
```
Stage 1:   自动化预检（4个Python脚本并行，38+9+7类检查）
Stage 1.5: 边界文本审查（LLM sub-agent，审查标题/图题/表题错别字）
Stage 2:   通读建档（LLM 略读全文，生成上下文档案）
Stage 3:   逐章深度审查（最多2并发sub-agent，清单驱动，实验章节强制读原始数据验算）
...
├── scripts/
│   ├── word_checker.py         # ★ Word 格式检查（38项）→ JSON
│   ├── format_checker.py       # PDF 空间检查（9项）→ JSON
│   ├── cross_ref_checker.py    # 交叉引用检查（7类）→ JSON
...
├── output/                     # 审计报告输出（按章节拆分）
├── examples/                   # 模板文件
│   └── 学术写作规范.template.md  # 写作规范模板（需复制并自定义）
├── 学术写作规范.md               # ★ LLM 唯一权威文档（本地，不入git）
├── docs/                       # 项目文档
├── requirements.txt
└── CLAUDE.md
```
...
- **PyMuPDF (fitz)** — PDF 空间坐标分析（版芯/题注位置/图片DPI）
...
| 能力范围 | 可靠性 | 说明 |
|---------|--------|------|
| 格式检查（Python 54项） | ✅ 100% 确定性 | 同输入同输出，0 误报 |
| 数据验算（Python 计算） | ✅ 100% 确定性 | 数学计算不会出错 |
| 交叉引用（Python 匹配） | ✅ 100% 确定性 | 引用存在性+顺序+未引用 |
