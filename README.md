...
硕士学位论文审计工具，作为 Claude Code Skill 运行。自动检查格式规范、内容质量和学术写作风格。

## 功能概览

| 审计线 | 执行者 | 检查内容 |
|--------|--------|---------|
| Word 格式检查 | Python + python-docx（34项） | 字体字号、标题层级/对齐/间距、页眉页脚、图表编号+中英文标题、公式、参考文献、空行、标点混用、括号匹配、重复字词 |
| PDF 空间检查 | Python + PyMuPDF（9项） | 页面尺寸、版芯、图表题注位置、页底空白、图片分辨率、公式居中、式号右对齐、表格跨页 |
...
### Word 格式检查项（34项，精确结构化数据）

| 类别 | 检查项 |
|------|--------|
| 字体 | 正文宋体12pt、英文Times New Roman、图题/表题五号宋体 |
| 标题 | 四级标题字号/字体/对齐/间距、摘要等特殊标题按一级排版、章标题≤20字 |
| 页眉 | 楷体10.5pt、左端固定文字、右端章号章题、起始页 |
| 页脚 | 页码12pt居中、·N·或-N-修饰 |
| 图表 | 编号按章连续、中英文双语标题（顺序+编号匹配）、题号间距、标题跨页检测 |
| 公式 | 编号按章连续、引用风格统一（式vs公式） |
| 参考文献 | [N]顺序编号、GB/T 7714格式标识（[M][J][D]） |
| 段落 | 末行≥5字、连续空行检测、编号分隔符半角点 |
| 目录 | 标题一致性、编号连续性 |
| 低级错误 | 中英文标点混用、全角半角数字混用、标题末尾多余标点、连续重复字词（"的的"）、括号不匹配、大表格跨页属性 |
...
├── scripts/
│   ├── word_checker.py         # ★ Word 格式检查（34项）→ JSON
│   ├── format_checker.py       # PDF 空间检查（9项）→ JSON
...
| 能力范围 | 可靠性 | 说明 |
|---------|--------|------|
| 格式检查（Python 43项） | ✅ 100% 确定性 | 同输入同输出，0 误报 |
| 数据验算（Python 计算） | ✅ 100% 确定性 | 数学计算不会出错 |
| 交叉引用（Python 匹配） | ✅ 100% 确定性 | 引用存在性检查 |
| 语病/润色（LLM 判断） | ⚠️ ~60% 概率性 | 每次可能关注不同句子 |
| 逻辑分析（LLM 理解） | ⚠️ ~70% 概率性 | 表面判断为主 |
| 数据造假 | ❌ 无法检测 | 只能验证论文值=CSV值，不能验证CSV本身是否真实 |
| 公式推导正确性 | ❌ 无法可靠检测 | LLM 不能验证密码学证明 |
| 抄袭/学术不端 | ❌ 无法检测 | 未接入查重系统 |
